...
contract ProductionPriceFeedConfig {
     ...
     */
    function getPriceFeedConfig(address token0, address token1)
        external
        pure
        returns (PriceFeedConfig memory config)
    {
        ...
        if (token0 == WETH && token1 == USDC) {
            return PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: ETH_USD_FEED, isActive: true
            });
        }
        ...
        if (token0 == WBTC && token1 == USDC) {
            return PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: BTC_USD_FEED, isActive: true
            });
        }
        ...
        if (token0 == LINK && token1 == USDC) {
            return PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: LINK_USD_FEED, isActive: true
            });
        }
        ...
        if (token0 == UNI && token1 == USDC) {
            return PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: UNI_USD_FEED, isActive: true
            });
        }
        ...
        if (token0 == AAVE && token1 == USDC) {
            return PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: AAVE_USD_FEED, isActive: true
            });
        }
        ...
        return
            PriceFeedConfig({
                token0: token0, token1: token1, priceFeed: address(0), isActive: false
            });
    }
