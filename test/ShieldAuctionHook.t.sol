...
contract ShieldAuctionHookTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        deployCodeTo(
            "ShieldAuctionHook",
            abi.encode(
                manager, avsDirectory, AVS_ADDRESS, priceOracle, FEE_RECIPIENT, LVR_THRESHOLD
            ),
            address(hook)
        );
    ...
    function test_endAuction_distributesRewards() public {
        ...
        assertTrue(
            block.timestamp >= auction.startTime + auction.duration, "Auction should have ended"
        );
    }
    ...
    function test_endAuction_afterSwap() public {
        ...
        assertTrue(
            ...
            "Auction should have ended by time"
        );
        ...
        assertTrue(
            auction.isComplete || !auction.isActive
                || block.timestamp >= auction.startTime + auction.duration,
    ...
    function test_afterAddLiquidity_tracksLiquidity() public {
        ...
        modifyLiquidityRouter.modifyLiquidity(
            poolKey,
            ModifyLiquidityParams({
                tickLower: -120, tickUpper: 120, liquidityDelta: int128(liquidityDelta), salt: 0
            }),
            ""
        );
    ...
    function test_afterRemoveLiquidity_handlesOverflow() public {
        ...
        modifyLiquidityRouter.modifyLiquidity(
            poolKey,
            ModifyLiquidityParams({
                tickLower: -120, tickUpper: 120, liquidityDelta: removeAmount, salt: 0
            }),
            ""
        );
