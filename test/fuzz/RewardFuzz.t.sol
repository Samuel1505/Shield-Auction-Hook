...
contract RewardFuzz is TestFixture {
    ...
    function testFuzz_rewardPercentagesConsistency(
        uint256 amount
    ) public {
        ...
        uint256 gasCompensation = (amount * hook.GAS_COMPENSATION_PERCENTAGE()) / hook.BASIS_POINTS();
    ...
    function testFuzz_lpRewardCalculation(
        uint256 totalAmount
    ) public {
    ...
    function testFuzz_operatorRewardCalculation(
        uint256 totalAmount
    ) public {
        totalAmount = bound(totalAmount, 1, type(uint128).max);
        
        uint256 expectedOperatorReward = (totalAmount * hook.AVS_REWARD_PERCENTAGE()) / hook.BASIS_POINTS();
    ...
    function testFuzz_protocolFeeCalculation(
        ...
        uint256 expectedProtocolFee = (totalAmount * hook.PROTOCOL_FEE_PERCENTAGE()) / hook.BASIS_POINTS();
    ...
    function testFuzz_gasCompensationCalculation(
        uint256 totalAmount
    ) public {
        totalAmount = bound(totalAmount, 1, type(uint128).max);
        
        uint256 expectedGasCompensation = (totalAmount * hook.GAS_COMPENSATION_PERCENTAGE()) / hook.BASIS_POINTS();
    ...
    function testFuzz_rewardDistributionZeroBid(
        uint256 seed
    ) public {
        bytes32 auctionId = createAuction();
    ...
    function testFuzz_rewardDistributionSmallBid(
        uint256 amount
    ) public {
    ...
    function testFuzz_rewardDistributionLargeBid(
        uint256 amount
    ) public {
    ...
    function testFuzz_multipleRewardDistributions(
        uint256 seed,
        uint8 numAuctions
    ) public {
        ...
        for (uint8 i = 0; i < numAuctions; i++) {
            ...
            uint256 winningBid = TestHelpers.createValidBidAmount(uint256(keccak256(abi.encode(seed, i))));
    ...
    function testFuzz_rewardRounding(
        uint256 amount
    ) public {
